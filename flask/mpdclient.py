...
class MyMPD(object):
    ...
    def connect(self):
        ...
        self.client.connect("secret.rocklosangeles.com", 6600)
        return self.client
...
class Dispatcher(object):
    ...
    def grab_title(vurl):
        try:
            ...
            return "Fail to get title."

    def do_youtube(self, url):
        data = {
	    'url': url,
            'format': 'bestaudio'
	}

        response = requests.post('http://secret.rocklosangeles.com:8080/youtube-dl/q', data=data) 
        return response
    ...
    def do_insert(self, playlist):
        '''Insert playlist after current playing song'''
        #file = '%s%s.m3u' % (PLAYLIST_DIR, playlist)
        #if not os.path.isfile(file):
        #    return "No such playlist (%s). Hint? Playlists are case-sensitive." % playlist
        try:
            #self.client.clear()
            self.client.insert("/var/lib/mpd/playlists/wfmu.m3u")
            #self.client.play()
            return "Playlist '%s' queued." % playlist
        except mpd.CommandError:
            self.client.insert("playlists/wfmu.m3u")
            #self.do_jukebox()
        return "No such playlist (%s). Hint? Playlists are case-sensitive." % playlist
